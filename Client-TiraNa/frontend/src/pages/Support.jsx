...
const STATUS_STYLES = {
  ...
  'in-progress': 'bg-sage/10 text-olive',
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="p-1">
                ...
                >
                  ...
                  <div className="text-left">
                    <p className={mode === 'form' ? 'text-white' : 'text-charcoal'}>Submit a Ticket</p>
                    <p className={`text-[11px] ${mode === 'form' ? 'text-white/60' : 'text-gray-400'}`}>Get help from our team</p>
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <div className="text-left">
                    ...
                    <p className={`text-[11px] ${mode === 'lookup' ? 'text-white/60' : 'text-gray-400'}`}>View your tickets</p>
                  </div>
                </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">Need Help?</h3>
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">support@tirana.ph</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
          ...
          <div className="lg:col-span-8">
            ...
            {mode === 'form' && (
              <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="px-6 sm:px-8 py-5 border-b border-gray-100 bg-gray-50/50">
                  <h2 className="text-lg font-semibold text-charcoal">Submit a New Ticket</h2>
                  <p className="text-sm text-gray-500 mt-1">Fill out the form below and our team will get back to you.</p>
                </div>

                <div className="p-6 sm:p-8 space-y-6">
                  {/* Contact Info */}
                  <div>
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">Contact Information</h3>
                  ...
                  {/* Ticket Details */}
                  <div>
                    <h3 className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-4">Ticket Details</h3>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <select
                              ...
                              value={form.category}
                              onChange={handleChange}
                              className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm text-charcoal appearance-none focus:outline-none focus:ring-2 focus:ring-sage/30 focus:border-sage transition-all duration-200 bg-white"
                            >
                              {CATEGORIES.map(c => (
                                <option key={c.value} value={c.value}>{c.label}</option>
                              ))}
                            </select>
                            <svg className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                            </svg>
                          </div>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-charcoal mb-2">Priority</label>
                          <div className="relative">
                            <select
                              name="priority"
                              value={form.priority}
            ...
            {mode === 'lookup' && (
              <div className="space-y-6">
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                  <div className="px-6 sm:px-8 py-5 border-b border-gray-100 bg-gray-50/50">
                    <h2 className="text-lg font-semibold text-charcoal">Check Your Ticket Status</h2>
                    <p className="text-sm text-gray-500 mt-1">Enter your email to view all your submitted tickets.</p>
                  </div>
                ...
                {tickets.length > 0 && (
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    <div className="px-6 sm:px-8 py-4 border-b border-gray-100 flex items-center justify-between">
                      <h3 className="text-sm font-semibold text-charcoal">Your Tickets</h3>
                            ...
                            <div className="flex items-start justify-between gap-4">
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="text-[11px] text-gray-400 font-mono bg-gray-100 px-2 py-0.5 rounded">#{ticket.id}</span>
                                  <span className={`inline-flex px-2 py-0.5 rounded text-[11px] font-medium ${STATUS_STYLES[ticket.status] || ''}`}>
                                    {ticket.status}
                                  </span>
                                </div>
                                <h4 className="text-sm font-medium text-charcoal mb-1">{ticket.subject}</h4>
                                <p className="text-xs text-gray-400">{formatDate(ticket.created_at)}</p>
                              </div>
                              <div className="flex items-center gap-2 shrink-0">
                                <span className={`inline-flex px-2 py-0.5 rounded text-[11px] font-medium ${PRIORITY_STYLES[ticket.priority] || ''}`}>
                                  {ticket.priority}
                                </span>
                                <svg className={`w-4 h-4 text-gray-300 transition-transform duration-200 ${selectedTicket?.id === ticket.id ? 'rotate-180' : ''}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
                                </svg>
                              </div>
                            </div>
                          ...
                          {selectedTicket?.id === ticket.id && (
                            <div className="px-6 sm:px-8 pb-6 bg-gray-50/30">
                              <div className="pt-4 border-t border-gray-100 space-y-4">
                                ...
                                {ticket.assigned_to && (
                                  <div>
                                    <p className="text-[11px] font-semibold uppercase tracking-wider text-gray-400 mb-2">Assigned To</p>
                                ...
                                {ticket.resolution && (
                                  <div>
                                    ...
                                    <p className="text-sm text-charcoal whitespace-pre-wrap leading-relaxed">{ticket.resolution}</p>
                                  </div>
                                )}
                                <div className="pt-2">
