...
export default function AdminDisputes() {
  ...
  const fetchDisputes = useCallback(async () => {
    setLoading(true)
    try {
      const data = await getDisputes({ status: statusFilter, limit: 100 })
        ...
        <div className="modal-overlay open" onClick={(e) => { if (e.target === e.currentTarget) setSelected(null) }}>
          <div className="modal modal-lg">
            ...
            <div className="info-grid">
              <div className="info-item"><p>Filed By</p><p>{selected.filed_by || '—'}{selected.filed_by_email ? ` (${selected.filed_by_email})` : ''}</p></div>
              <div className="info-item"><p>Status</p><p><span className={`badge badge-${selected.status}`}>{selected.status}</span></p></div>
            ...
            <div className="modal-footer">
              <button className="btn btn-ghost" onClick={() => setSelected(null)} disabled={saving}>Cancel</button>
              <button className="btn btn-brand" onClick={handleUpdate} disabled={saving}>{saving ? 'Saving...' : 'Save Changes'}</button>
            </div>
          </div>
        </div>
      )}
